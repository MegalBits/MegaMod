...
import net.mcreator.megamodmain.block.PurityLogBlock;
import net.mcreator.megamodmain.block.PurityLeavesBlock;
...
public class TerrariaPurityBiome extends MegamodmainModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(PurityLogBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(PurityLeavesBlock.block.getDefaultState()))).baseHeight(4)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == TerraGrassBlock.block.getDefaultState().getBlock()
							|| ground == TerraDirtBlock.block.getDefaultState().getBlock())
							&& (ground2 == TerraGrassBlock.block.getDefaultState().getBlock()
									|| ground2 == TerraDirtBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), TerraDirtBlock.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == PurityLeavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, PurityLeavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, PurityLogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == PurityLogBlock.block.getDefaultState().getBlock()
					|| blockType == PurityLeavesBlock.block.getDefaultState().getBlock()
					|| blockType == TerraGrassBlock.block.getDefaultState().getBlock()
					|| blockType == TerraDirtBlock.block.getDefaultState().getBlock();
		}
